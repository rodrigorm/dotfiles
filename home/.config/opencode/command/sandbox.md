...
Run `retry` using the role listed in `allowedActions`. A remote capability must not retry a failed `start` or a forced delete; report that host recovery is required instead. `error.retryable` is advisory; never retry unless `allowedActions` includes the typed `retry` action.

Run `sandboxctl $ARGUMENTS` exactly once. Treat its single JSON document on stdout as authoritative. Disk records use `schemaVersion: 1`; normal responses use `schemaVersion: 2`. Present `schemaVersion`, `requestId`, `ok`, `operation`, `message`, `session`, `intent`, `effectiveTarget`, `observations`, `classification`, `work`, `allowedActions`, `recommendedAction`, and `error`, plus compatibility `state`, `stage`, and non-secret `details` when present. Preserve every `observations` entry, including `observed: false`; do not infer provider health, resource existence, ownership, target, or completed cleanup from persisted intent, compatibility state, or prose. Do not automatically execute a recommended action; report its role, preconditions, and wait behavior.

Never infer or execute provider infrastructure commands (`sbx`, exe.dev/SSH, Cloudflare, or other provider tools), access a control socket directly, construct infrastructure commands, or retry an operation yourself. Cloudflare permits only live inspection of a known resource; it has no durable lookup, inventory, or runtime-adoption path after restart, so do not invent one. If the JSON contains `diagnosticOperation: "diagnose"`, run only `sandboxctl diagnose` exactly once and present that result.
